...
| Workbench layout | Done. **The masthead** carries the mark on the left and, on the right, two separate buttons: **Save font** (a bookmark that fills and reads Saved when this font is kept; pressing again forgets) and **Saved fonts · N**, the door to the room — named for the room, so it cannot be read as a second Save, hidden at zero. **The sticky bar is three zones on one line** — the name on the left, **Base font and Style as one divided control** centred on the page, Compose and Download on the right — 70 px tall where it was 151. The glyphs-and-licence line and the report of the last download live in the Download menu's foot. It holds one line down to about 700 px, then the name takes a line to itself, and on a phone every zone gets its own. presets above the plate as pictures of themselves; layers as cards, **each with its own Reset**; every dial visible — the four longest treatments breaking into named runs rather than hiding half of themselves — and the size ladder labelled above each line. **Randomise is gone** and the plate's footer with it. See DECISIONS. |
